...
def calculate_lastdim_num_blocks(input_tensor: torch.Tensor, block_size: int) -> int:
    ...
    num_elements = numel // stride  # number of coordinate points
    total_blocks = math.ceil(num_elements / block_size)
    return total_blocks
...
class FifthOrderSphericalHarmonic(torch.autograd.Function):
    ...
    def forward(
        ...
        # Dynamically adjust block_size to maintain parallelism
        num_coords = coord_numel // coords.stride(-2)
        MIN_BLOCKS = 64  # Minimum number of blocks for good GPU utilization
        if num_coords < block_size * MIN_BLOCKS:
            block_size = max(1, num_coords // MIN_BLOCKS)
            # Round to next power of 2 for efficiency
            block_size = max(1, 1 << (block_size - 1).bit_length()) if block_size > 1 else 1
    ...
    def backward(
        ...
        return coord_grad_output, None, None, None, None
...
def test_fifth_order_spherical_harmonic():
    # Generate random input coordinates with gradient tracking
    coords = torch.randn(128, 3, device='cuda', dtype=torch.float32, requires_grad=True)
    ...
    for i, (block_size, col_offset) in enumerate([(64, 0), (32, 1), (128, 2), (256, 3)], start=1):
        ...
        # Trigger backward pass to profile fifth_order_bwd kernel
        triton_output.sum().backward(retain_graph=True)
