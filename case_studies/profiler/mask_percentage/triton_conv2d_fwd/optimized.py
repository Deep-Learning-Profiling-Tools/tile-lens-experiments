...
def conv2d_forward(input_tensor, weight_tensor, kernel_height, kernel_width, stride_height, stride_width, padding_height, padding_width, groups, fp16=False, tf32=False):
    ...
    # Define block sizes dynamically
    in_group_dim = in_feat_dim // groups
    out_group_dim = out_feat_dim // groups
    BLOCK_SIZE_BATCH_HEIGHT_WIDTH = max(32, min(128, triton.next_power_of_2(batch_dim * out_height * out_width)))
    BLOCK_SIZE_IN_FEAT = max(16, min(32, triton.next_power_of_2(in_group_dim)))
    BLOCK_SIZE_OUT_FEAT = max(16, min(32, triton.next_power_of_2(out_group_dim)))
