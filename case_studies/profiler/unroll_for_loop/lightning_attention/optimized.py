...
def _fwd_kernel(
    ...
    #     off_block += BLOCK
    # ======= after optimized =========
    # Precompute offsets for 2 iterations
    off_block_0 = off_block
    off_block_1 = off_block + BLOCK

    # Precompute masks
    mask_q_0 = off_block_0[:, None] < n
    mask_k_0 = off_block_0[None, :] < n
    mask_v_0 = off_block_0[:, None] < n
    mask_q_1 = off_block_1[:, None] < n
    mask_k_1 = off_block_1[None, :] < n
    mask_v_1 = off_block_1[:, None] < n

    # Load all data
    q_0 = tl.load(Q_block_ptr + off_block_0[:, None] * d, mask=mask_q_0, other=0.0).to(tl.float32)
    k_trans_0 = tl.load(K_trans_block_ptr + off_block_0[None, :] * d, mask=mask_k_0, other=0.0).to(tl.float32)
    v_0 = tl.load(V_block_ptr + off_block_0[:, None] * e, mask=mask_v_0, other=0.0).to(tl.float32)
    q_1 = tl.load(Q_block_ptr + off_block_1[:, None] * d, mask=mask_q_1, other=0.0).to(tl.float32)
    k_trans_1 = tl.load(K_trans_block_ptr + off_block_1[None, :] * d, mask=mask_k_1, other=0.0).to(tl.float32)
    v_1 = tl.load(V_block_ptr + off_block_1[:, None] * e, mask=mask_v_1, other=0.0).to(tl.float32)

    # Iteration 0: compute
    qk_0 = tl.dot(q_0, k_trans_0)
    qk_0 = tl.where(index >= 0, qk_0, 0)
    o_intra_0 = tl.dot(qk_0, v_0)
    o_inter_0 = tl.dot(q_0, kv)
    o_0 = o_intra_0 + o_inter_0
    kv += tl.dot(k_trans_0, v_0)

    # Iteration 1: compute
    qk_1 = tl.dot(q_1, k_trans_1)
    qk_1 = tl.where(index >= 0, qk_1, 0)
    o_intra_1 = tl.dot(qk_1, v_1)
    o_inter_1 = tl.dot(q_1, kv)
    o_1 = o_intra_1 + o_inter_1
    kv += tl.dot(k_trans_1, v_1)

    # Store all results
    tl.store(O_block_ptr + off_block_0[:, None] * e, o_0.to(O_block_ptr.dtype.element_ty), mask=mask_v_0)
    tl.store(O_block_ptr + off_block_1[:, None] * e, o_1.to(O_block_ptr.dtype.element_ty), mask=mask_v_1)
    # ====================================
